...
function replaceOnce(string $text, string $old, string $new, string $label): string
{
    ...
    if ($count !== 1) {
        throw new RuntimeException("{$label}: expected exactly one replacement, got {$count}");
    }
...
PHP;
$source = replaceOnce($source, $oldConnection, $newConnection, 'queue connection semantics');

$source = replaceOnce(
    $source,
    "        return preg_match('/->\\s*afterCommit\\s*\\(/i', \$statement) === 1;",
    "        return preg_match('/->\\s*afterCommit\\s*\\(/i', \$this->codeOnlyFragment(\$statement)) === 1;",
    'afterCommit semantic check',
);
$source = replaceOnce(
    $source,
    "        if (preg_match('/::\\s*'.preg_quote(\$callMethod, '/').'\\s*\\(/i', \$statement, \$match, PREG_OFFSET_CAPTURE) !== 1) {",
    "        if (preg_match('/::\\s*'.preg_quote(\$callMethod, '/').'\\s*\\(/i', \$this->codeOnlyFragment(\$statement), \$match, PREG_OFFSET_CAPTURE) !== 1) {",
    'call preference dispatch lookup',
);
$source = replaceOnce(
    $source,
